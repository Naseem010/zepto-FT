...
const initialState = {
  allUser: data,
  addedUser: [],
  matchedUser: data.slice(0, Math.min(5, data.length)),
...
export const mailSlice = createSlice({
  ...
  reducers: {
    ...
    addUser: (state, action) => {
      ...
      state.matchedUser = state.allUser.slice(
        0,
        Math.min(5, state.allUser.length)
      );
    ...
    removeUser: (state, action) => {
      ...
      if (userId === -1) {
        ...
        const userIndex = state.addedUser.findIndex(
          (user) => user.id === userId
        );
        if (userIndex !== -1) {
          const removedUser = state.addedUser.splice(userIndex, 1)[0];
...
export const { setMatchedUser, addUser, removeUser, setHighlightLastOne } =
  mailSlice.actions;
